    ...
    useEffect(() => {
        ...
        peer.on('error', (err) => {
            ...
            if (err.type === 'peer-unavailable') {
                // Host might be temporarily offline (reloading), so we don't set a hard error.
                // We just ensure we are marked as disconnected so the loop retries.
                setIsConnected(false);
                connRef.current = null;
            } else if (err.type === 'unavailable-id') {
                // Similar to peer-unavailable
                setIsConnected(false);
                connRef.current = null;
            } else {
                // For other errors, just mark disconnected, let heartbeat retry
                setIsConnected(false);
            }
        });
        ...
        const interval = setInterval(() => {
            ...
            // Fatal errors stop retries (only explicit game over/lock states)
            if (error && (error.includes("Locked") || error.includes("ended"))) return;

            // If we have a generic "Host not found" error, clear it to allow retry
            if (error === "Host not found. Check room code.") {
                setError(null);
            }
            ...
            if (!connRef.current || !connRef.current.open) {
                console.log('Heartbeat: Connection lost, retrying host...');
